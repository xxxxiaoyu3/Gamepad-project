...
void MX_TIM3_Init(void)
{
  ...
  // 启动定时器3同时开启中断 Start the timer3 and enable interrupt
  if (HAL_TIM_Base_Start_IT(&htim3) != HAL_OK)  // 启动定时器中断
  {
  // 启动失败，可以在这里添加错误处理代码，比如点亮一个错误指示灯
  Error_Handler();
  }
